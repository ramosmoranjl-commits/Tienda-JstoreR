...
function handleLocation() {
    ...
    navigator.geolocation.getCurrentPosition(pos => {
        const dist = getDist(STORE_LOCATION.lat, STORE_LOCATION.lng, pos.coords.latitude, pos.coords.longitude);
        document.getElementById('form-km').value = dist.toFixed(1);
        document.getElementById('gps-icon').className = 'fas fa-check-circle';
        document.getElementById('gps-icon').style.color = '#10b981';
        if(!document.getElementById('form-address').value) document.getElementById('form-address').value = "Ubicación GPS";
        validateForm();
        refreshSummary();
    }, () => {
        alert("Permiso denegado. Ingresa los KM manualmente (aprox).");
        document.getElementById('form-km').removeAttribute('readonly');
    });
}
